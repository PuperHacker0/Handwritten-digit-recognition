...
class  Model():
    def __init__(self):
        ...
        self.model.add(Dense(self.num_pixels, input_shape=(self.num_pixels,), kernel_initializer='normal', activation='relu'))
    ...
    def load_data(self):
        ...
        (self.train_image_data, self.train_image_labels), (self.test_image_data, self.test_image_labels) = mnist.load_data()
        
        #Flatten 28*28 image data to 784 vector
        self.train_image_data = self.train_image_data.reshape((self.train_image_data.shape[0], self.num_pixels)).astype('float32')
        self.test_image_data = self.test_image_data.reshape((self.test_image_data.shape[0], self.num_pixels)).astype('float32')
    ...
    def train(self):
        ...
        self.model.fit(self.train_image_data, self.train_image_labels, validation_data=(self.test_image_data, self.test_image_labels), epochs = 20, batch_size=200, verbose=2)
...
class Application():
    def __init__(self, window):
        ...
        self.window.title("Digit recognition application")
    ...
    def create_canvas(self):
        ...
        self.white_color_RGB = (255, 255, 255)
        self.overlay_image = Image.new("RGB", (self.can_width, self.can_height), self.white_color_RGB)
        self.canvasOverlay = ImageDraw.Draw(self.overlay_image)
    ...
    def draw_line(self, event):
        ...
        self.canvasOverlay.line([self.lastx, self.lasty, event.x, event.y], black, width = pen_thickness)
